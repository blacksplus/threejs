...
<script>
    ...
    // import fragmentShader from '@/assets/shader/missile/fragmentShader.glsl'
    export default {
        name: "missile",
        mounted() {
            ...
            //设置灯光位置
            directionLight1.position.set(0, 0, 20)
            ...
            let path, missile, iTime = {
                value: 0
            }
            const gltfLoader = new GLTFLoader()
            gltfLoader.load("/integrated/missile/ew8.glb", (gltf) => {
                // scene.add(gltf.scene)
                console.log(gltf.scene)
                const mapwkl = gltf.scene.children[0]
                const mapels = gltf.scene.children[1]
                ...
                const pathPoints = []
                console.log(path.geometry.attributes.position)
...
{  
    ...
    vec2 uvCoord = vec2(uv.x*iResolution.x,uv.y*iResolution.y);
	// ro: ray origin
	// rd: direction of the ray
	vec3 rd = normalize(vec3(2.0*(uvCoord-0.5*iResolution.xy)/iResolution.xy, 1.));
    ...
    gl_FragColor = vec4(sum.xyz,1.0);
                ...
                shader.uniforms.iChannel0 = {
                    value: new THREE.TextureLoader().load('/integrated/missile/iChannel0.png')
                }
                shader.uniforms.iChannel1 = {
                    value: new THREE.TextureLoader().load('/integrated/missile/iChannel1.png')
                }
                shader.uniforms.iChannel2 = {
                    value: new THREE.TextureLoader().load('/integrated/missile/iChannel2.png')
                }
            }
            ...
            function animation() {
                ...
                let t = time / 5
                if (path && t < 1) {
                    missile.position.copy(path.getPoint(t))
                    missile.lookAt(path.getPoint(t + 0.01))
                }
                iTime.value = t * 10
